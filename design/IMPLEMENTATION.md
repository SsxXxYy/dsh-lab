...
### 4.1 执行工作流（LLM 驱动的多轮工具调用）

```
用户: "帮我执行 DG 双通道直流输出工作流"

Turn 1: systemPrompt 显示工作流索引 → 知道有 dg_dc_output
        → read_workflow(name="dg_dc_output")
        → lab-local.ts 读文件（TypeScript 直接处理）
        → 返回工作流文件内容

Turn 2: LLM 阅读工作流内容，理解步骤
        → send_scpi(address="", command=":SOUR1:APPL:DC 100,5,2,0")
        → lab-local.ts → python -m dsh_lab.scpi
        → 返回 "SCPI 写入成功"

Turn 3: → send_scpi(address="", command=":SOUR2:APPL:DC 100,5,2,0")
        → 返回 "SCPI 写入成功"
...
- [ ] `src/lab-local.ts` 实现所有方法（文件操作 + Python 调用）
- [ ] `src/tools.ts` 新建，注册 8 个工具
- [ ] `src/context.ts` 新建，注册 3 个 system prompt section
- [ ] 斜杠命令 `/devices` `/new` `/rename` 注册
- [ ] Python 引擎 `dsh_lab/scan.py`、`dsh_lab/scpi.py`、`dsh_lab/asg.py` 实现
