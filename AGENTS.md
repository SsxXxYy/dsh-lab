...
> **目标**：让 LLM 快速、准确地理解意图，减少歧义和冗余。

### 7.1 官方 DSH 风格特征

| 特征 | 做法 | 反例 |
|---|---|---|
| **结构化** | 表格、无序列表、代码块 | 大段叙述文字 |
| **简洁** | 一句话能说清的不写两句 | 重复解释同一概念 |
| **定义式** | 直接定义"是什么"、"怎么做" | 铺垫式、故事化叙述 |
| **无冗余** | 同一信息只出现一次 | 多处重复相同内容 |
| **层级清晰** | `##` 大节 → `###` 子节 → 列表 | 全是 `##`，扁平化 |
| **参数表格** | 用表格列出参数名、说明、是否必选 | 用 JSON 代码块 + 叙述 |

### 7.2 编写规则

1. **角色定位**：一句话定义身份 + 能力列表（无序列表）
2. **意图判断**：用表格列出"请求类型 → 处理方式"
3. **操作流程**：用编号列表，步骤不超过 6 步
4. **行为准则**：用无序列表，每条独立、不重复
5. **工具说明**：每个工具一个小节，参数用表格
6. **注意事项**：用无序列表，放在最后
...
```
向仪器发送 SCPI 命令，每条命令需要 address 参数表示设备地址，
command 参数表示命令内容，delay 参数表示延迟时间...
```
